...
// client handlers
func MakeConnectedHandler(config Config) func(mqtt.Client) {
	return func(client mqtt.Client) {
		fmt.Println("Connected")
		sub(client, config.TargetTopic, MakeMessageHandler(config))
	}
}

func MakeConnectionLostHandler() func(mqtt.Client, error) {
	return func(client mqtt.Client, err error) {
		fmt.Printf("Connection lost: %v\n", err)
	}
}

func MakeMessageHandler(config Config) func(mqtt.Client, mqtt.Message) {
